...
XData <- XData[, MeanStandardDeviation]

# Set column names
names(XData) <- Features[MeanStandardDeviation, 2]
...
#### 5. From the data set in step 4, creates a second, independent tidy data set with the average of each variable for each activity and each subject.
AVG_TidyData <- ddply(TidyData, .(subject, activity), function(x) colMeans(x[, 1:66]))

write.table(AVG_TidyData, "AVG_TidyData.txt", row.name=FALSE)
